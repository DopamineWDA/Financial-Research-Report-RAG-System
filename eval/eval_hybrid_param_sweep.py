...
def parse_args() -> argparse.Namespace:
    ...
    parser = argparse.ArgumentParser(
        description="Sweep hybrid retrieval parameters on fixed FAISS Flat chunked indexes."
    )
    parser.add_argument(
        "--index-dirs",
        type=Path,
    ...
    parser.add_argument("--batch-size", type=int, default=16, help="Dense embedding batch size.")
    parser.add_argument("--vector-candidate-k", type=int, default=50, help="Vector candidate pool for hybrids.")
    parser.add_argument("--bm25-candidate-k", type=int, default=50, help="BM25 candidate pool for hybrids.")
    ...
    parser.add_argument(
        "--output-md",
        type=Path,
        default=Path(__file__).resolve().parents[1] / "eval" / f"{Path(__file__).stem}.md",
...
def sweep_weightsum(
    *,
    ...
    vector_candidate_ids: np.ndarray,
    vector_candidate_scores: np.ndarray,
    bm25_score_rows: list[np.ndarray],
    samples: list[dict],
    records: list[dict],
    top_k: int,
    ...
    for vector_weight, bm25_weight in weight_pairs:
        ...
        for qi, scores in enumerate(bm25_score_rows):
            vector_top = [int(idx) for idx in vector_candidate_ids[qi][:vector_candidate_k] if int(idx) >= 0]
            bm25_top = top_indices(scores, bm25_candidate_k)
            merged_ids: list[int] = []
            seen = set()
            ...
            vector_score_map = normalize_score_map(
                {
                    ...
                    for score, idx in zip(
                        vector_candidate_scores[qi][:vector_candidate_k],
                        vector_candidate_ids[qi][:vector_candidate_k],
                    )
                    if int(idx) >= 0
                }
            )
            bm25_score_map = normalize_score_map({idx: float(scores[idx]) for idx in bm25_top})
            ...
            ranked_lists.append([idx for _, idx in fused[:top_k]])

        rows.append(
            {
                "index": index_name,
                "retrieval": f"hybrid_weightsum({vector_weight:.1f}/{bm25_weight:.1f})",
                **evaluate_route(samples, records, ranked_lists),
            }
        )
    return rows
    ...
    *,
    ...
    for rrf_k in rrf_ks:
        ...
        rows.append(
            {
                "index": index_name,
                "retrieval": f"hybrid_rrf(k={rrf_k})",
                **evaluate_route(samples, records, ranked_lists),
            }
        )
    return rows
    ...
    for index_dir in args.index_dirs:
        ...
        rows.extend(
            sweep_weightsum(
                ...
                bm25_score_rows=bm25_score_rows,
                samples=samples,
                records=records,
                top_k=args.top_k,
