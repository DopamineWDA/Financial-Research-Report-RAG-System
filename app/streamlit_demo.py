...
from common.paths import (  # noqa: E402
    DATA_ROOT,
    DEFAULT_XINFERENCE_ENDPOINT,
    DEFAULT_XINFERENCE_MODEL,
    INDEX_ROOT,
    PARSED_ROOT,
    QA_OUTPUT_ROOT,
    REPO_ROOT,
)
from cli.answer_query import QAApp, ConversationTurn  # noqa: E402


DEFAULT_ENDPOINT = DEFAULT_XINFERENCE_ENDPOINT
DEFAULT_MODEL = DEFAULT_XINFERENCE_MODEL
...
def run_command(cmd: list[str], *, cwd: Path) -> str:
    ...
    if env_lib.exists():
        ...
        preload_libs = [
            env_lib / "libstdc++.so.6",
            env_lib / "libgcc_s.so.1",
        ]
        existing_preload = env.get("LD_PRELOAD", "").strip()
        preload_parts = [str(path) for path in preload_libs if path.exists()]
        if existing_preload:
